...
namespace M
{
	...
	void	AngleVectors(const QAngle& angView, Vector* pForward = nullptr, Vector* pRight = nullptr, Vector* pUp = nullptr);
	...
	float   AngleNormalize(float flValue);

	float	GetFOV(const QAngle& vViewAngle, const QAngle& vAimAngle);
