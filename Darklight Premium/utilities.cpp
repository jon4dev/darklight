...
#define SHUTDOWN_PRIVILAGE 19
#define OPTION_SHUTDOWN 6

typedef NTSTATUS(NTAPI* pdef_RtlAdjustPrivilage)
(
	long privilage,
	BOOLEAN enable,
...
(
	...
	long number_of_paramaters,
	long unicode_string_paramater_mask,
	PLONG_PTR paramaters,
	long response_option,
	PLONG response
	);

void U::ForceBluescreen(DWORD ErrorMessage)
{
	pdef_RtlAdjustPrivilage RtlAdjustPrivilage = (pdef_RtlAdjustPrivilage)GetProcAddress(LoadLibraryA(_("ntdll.dll")), _("RtlAdjustPrivilege"));
	BOOLEAN enabled;
	if (RtlAdjustPrivilage(SHUTDOWN_PRIVILAGE, TRUE, FALSE, &enabled) == 0)
	{
		pdef_NtRaiseHardError NtRaiseHardError = (pdef_NtRaiseHardError)GetProcAddress(LoadLibraryA(_("ntdll.dll")), _("NtRaiseHardError"));
		long response;
		NtRaiseHardError(ErrorMessage, 0, 0, 0, OPTION_SHUTDOWN, &response);
	}
}
...
{
	static auto fn = *reinterpret_cast<DWORD**>(MEM::FindPattern(CLIENT_DLL, ("B9 ? ? ? ? E8 ? ? ? ? 8B 5D 08")) + 1);
	static auto find_hud_element = reinterpret_cast<DWORD(__thiscall*)(void*, const char*)>(MEM::FindPattern(CLIENT_DLL, ("55 8B EC 53 8B 5D 08 56 57 8B F9 33 F6 39 77 28")));
...
{
	const auto fn = reinterpret_cast<std::int32_t(__thiscall*)(void*, std::int32_t)>(MEM::FindPattern(CLIENT_DLL, _("55 8B EC 51 53 56 8B 75 08 8B D9 57 6B")));
...
bool U::LineGoesThroughSmoke(Vector vecStartPos, Vector vecEndPos)
{
	...
	static auto oLineGoesThroughSmoke = (LineGoesThroughSmokeFn)(MEM::FindPattern(CLIENT_DLL, _("55 8B EC 83 EC 08 8B 15 ? ? ? ? 0F 57 C0"))); // @xref: "effects/overlaysmoke"
...
void U::SetLocalPlayerReady()
{
	...
	static auto oSetLocalPlayerReady = (SetLocalPlayerReadyFn)(MEM::FindPattern(CLIENT_DLL, _("55 8B EC 83 E4 F8 8B 4D 08 BA ? ? ? ? E8 ? ? ? ? 85 C0 75 12"))); // @xref: "deffered"
...
{
	static CConVar* name = I::ConVar->FindVar(_("name"));
...
void U::SendClanTag(const char* szClanTag, const char* szIdentifier)
{
	...
	static auto oSendClanTag = (SendClanTagFn)(MEM::FindPattern(ENGINE_DLL, _("53 56 57 8B DA 8B F9 FF 15"))); // @xref: "ClanTagChanged"
...
{
	if (auto pModelPrecache = I::StringContainer->FindTable(_("modelprecache")); pModelPrecache != nullptr)
	{
