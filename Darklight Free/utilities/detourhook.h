...
public:
	...
	bool Create(void* pFunction, void* pDetour)
	{
		...
		{
			throw std::runtime_error(fmt::format(_("failed to create hook function, status: {:d}\nbase function -> {:#08X}"), (int)status, (std::uintptr_t)pBaseFn));
			return false;
		}
	...
	bool Replace()
	{
		...
		{
			throw std::runtime_error(fmt::format(_("failed to enable hook function, status: {:d}\nbase function -> {:#08X} address"), (int)status, (std::uintptr_t)pBaseFn));
			return false;
		}
	...
	bool Remove()
	{
		...
		{
			throw std::runtime_error(fmt::format(_("failed to remove hook, status: {:d}\n base function -> {:#08X} address"), (int)status, (std::uintptr_t)pBaseFn));
			return false;
		}
	...
	bool Restore()
	{
		...
		{
			throw std::runtime_error(fmt::format(_("failed to restore hook, status: {:d}\n base function -> {:#08X} address"), (int)status, (std::uintptr_t)pBaseFn));
			return false;
		}
