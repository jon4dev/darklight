...
bool CBacktrack::ValidTick(float flTime) {
	INetChannelInfo* pNetChan = I::Engine->GetNetChannelInfo();
	if (!pNetChan)
		return false;

	auto flDelta = std::clamp(
		pNetChan->GetLatency(0) + LerpTime(),
		0.f,
		I::ConVar->FindVar(_("sv_maxunlag"))->GetFloat()) - (I::Globals->flCurrentTime - flTime);
	return std::fabsf(flDelta) <= 0.2f;
}
