...
namespace GUI
{
	...
	namespace CONTROLS
	{
		...
		void BeginGroupbox(const std::string& szName, const int iColumn = 0);
		void EndGroupbox();

		void Checkbox(const std::string& szName, bool* pValue);

		void Slider(const std::string& szName, int* pValue, int iMin, int iMax);
		void Slider(const std::string& szName, float* pValue, float flMin, float flMax);
		...
		void Keybind(const std::string& szName, int* pBind, int* pMethod, bool bAllowMethod = true);
		void KeybindFocus();

		void TextField(const std::string& szName, std::string* pValue);
		void NumberField(const std::string& szName, std::string* pValue);

		void Colorpicker(const std::string& szName, Color* pColor, uint8_t* pAlpha, const bool bAlpha = false, const int iColumn = 0);
